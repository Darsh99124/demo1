...
def Message prepareUnitOfMeasureFilterQuery(Message message) {
  ...
  def uomFilter = "\$filter=UnitOfMeasureISOCode%20ne%20%27%27%20" + (dimensions ? "and%20($dimensionsFilter)" : "") + (sapClient ? "&sap-client=$sapClient" : "")
  def isoCodeFilter = "\$filter=UnitOfMeasureISOCodeName%20ne%20%27%27" + (languages ? "and%20($languageFilter)" : "") + (sapClient ? "&sap-client=$sapClient" : "")
  def commercialNameFilter = "\$filter=UnitOfMeasureCommercialName%20ne%20%27%27" + (languages ? "and%20($languageFilter)" : "") + (sapClient ? "&sap-client=$sapClient" : "")
