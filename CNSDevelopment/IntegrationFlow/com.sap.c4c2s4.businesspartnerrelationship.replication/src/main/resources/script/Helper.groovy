...
def Message enrichJsonPayload(Message message) {
  ...
  def businessPartnerRelationshipS4ReplicationOutOpenAPISchemaStream = this.getClass().getResourceAsStream('/src/main/resources/json/businessPartnerRelationshipS4ReplicationOut.json')
  try {
    def schema = new JsonSlurper().parse(businessPartnerRelationshipS4ReplicationOutOpenAPISchemaStream)
      .components.schemas.BusinessPartnerRelationshipOutboundReplicationcreaterequest.properties.messageRequests.items.properties.body
    ...
    if (businessPartnerRelationshipS4ReplicationOutOpenAPISchemaStream != null) {
      businessPartnerRelationshipS4ReplicationOutOpenAPISchemaStream.close()
    }
  }
  return message;
}
