...
def Message enrichJsonOutput(Message message) {
  ...
  parsedJson.messageRequests.each {
    ...
    if (it.body.salesOrganizationDisplayId.length() == 0) 
      it.body.salesOrganizationDisplayId = null    

    if (it.body.distributionChannel.length() == 0) 
      it.body.distributionChannel = null 
   
    if (it.body.division.length() == 0) 
      it.body.division = null

    if (it.body.salesOfficeDisplayId.length() == 0) 
      it.body.salesOfficeDisplayId = null
      
    if (it.body.salesGroupDisplayId.length() == 0) 
      it.body.salesGroupDisplayId = null      

    if (it.body.parentInstallPointDisplayId.length() == 0) 
      it.body.parentInstallPointDisplayId = null     
  
    if (it.body.parentRegisteredProductDisplayId.length() == 0) 
      it.body.parentRegisteredProductDisplayId = null   
